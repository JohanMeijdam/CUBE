...
DO $BODY$
	...
	BEGIN
		...
		DELETE FROM t_function WHERE cube_id = 'CUBE-NULL';
		DELETE FROM t_argument WHERE cube_id = 'CUBE-NULL';
	END;
$BODY$;
		...
		LOOP
			...
			WHEN 'bot' THEN
				FOR rec_sequence IN
					...
					WHERE sequence_catalog = rec_schema.catalog_name
					  AND sequence_schema = rec_schema.schema_name
					  AND sequence_name NOT IN ('sq_bot','sq_typ','sq_tsg','sq_tsp','sq_atb','sq_der','sq_dca','sq_rta','sq_ref','sq_dcr','sq_rtr','sq_rts','sq_srv','sq_sst','sq_svd','sq_rtt','sq_jsn','sq_dct')
				LOOP
					EXECUTE 'DROP SEQUENCE ' || rec_schema.schema_name || '.' || rec_sequence.sequence_name;
				END LOOP;
				FOR rec_table IN
					...
					WHERE table_type = 'BASE TABLE'
					  AND table_catalog = rec_schema.catalog_name
					  AND table_schema = rec_schema.schema_name
					  AND table_name NOT IN ('t_business_object_type','t_type','t_type_specialisation_group','t_type_specialisation','t_attribute','t_derivation','t_description_attribute','t_restriction_type_spec_atb','t_reference','t_description_reference','t_restriction_type_spec_ref','t_restriction_target_type_spec','t_service','t_service_step','t_service_detail','t_restriction_type_spec_typ','t_json_path','t_description_type')
				LOOP
					EXECUTE 'DROP TABLE ' || rec_schema.schema_name || '.' || rec_table.table_name;
				END LOOP;
				FOR rec_table IN
					...
					WHEN 't_service_detail' THEN
						FOR rec_column IN
							...
							WHERE table_catalog = rec_schema.catalog_name
							  AND table_schema = rec_schema.schema_name
							  AND table_name = rec_table.table_name 
							  AND column_name NOT IN ('cube_id','fk_bot_name','fk_typ_name','fk_srv_name','fk_srv_cube_tsg_db_scr','cube_tsg_atb_ref','xf_atb_typ_name','xk_atb_name','xk_ref_bot_name','xk_ref_typ_name','xf_ref_typ_name','xk_ref_sequence')
						LOOP
							EXECUTE 'ALTER TABLE ' || rec_schema.schema_name || '.' || rec_table.table_name || ' DROP COLUMN ' || rec_column.column_name || ' CASCADE';
						END LOOP;
		...
		LOOP
			...
			WHEN 'bot' THEN
				...
				WHEN 't_service_detail' THEN
					...
					WHEN 'cube_id' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(16)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN cube_id TO _cube_id' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN cube_id DROP DEFAULT';
						END IF;			
					WHEN 'fk_bot_name' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(30)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN fk_bot_name TO _fk_bot_name' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN fk_bot_name DROP DEFAULT';
						END IF;			
					WHEN 'fk_typ_name' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(30)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN fk_typ_name TO _fk_typ_name' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN fk_typ_name DROP DEFAULT';
						END IF;			
					WHEN 'fk_srv_name' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(30)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN fk_srv_name TO _fk_srv_name' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN fk_srv_name DROP DEFAULT';
						END IF;			
					WHEN 'fk_srv_cube_tsg_db_scr' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(8)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN fk_srv_cube_tsg_db_scr TO _fk_srv_cube_tsg_db_scr' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN fk_srv_cube_tsg_db_scr SET DEFAULT ''D''';
						END IF;			
					WHEN 'cube_tsg_atb_ref' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(8)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN cube_tsg_atb_ref TO _cube_tsg_atb_ref' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN cube_tsg_atb_ref SET DEFAULT ''ATB''';
						END IF;			
					WHEN 'xf_atb_typ_name' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(30)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN xf_atb_typ_name TO _xf_atb_typ_name' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN xf_atb_typ_name DROP DEFAULT';
						END IF;			
					WHEN 'xk_atb_name' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(30)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN xk_atb_name TO _xk_atb_name' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN xk_atb_name DROP DEFAULT';
						END IF;			
					WHEN 'xk_ref_bot_name' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(30)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN xk_ref_bot_name TO _xk_ref_bot_name' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN xk_ref_bot_name DROP DEFAULT';
						END IF;			
					WHEN 'xk_ref_typ_name' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(30)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN xk_ref_typ_name TO _xk_ref_typ_name' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN xk_ref_typ_name DROP DEFAULT';
						END IF;			
					WHEN 'xf_ref_typ_name' THEN
						IF rec_column.cube_data_type <> 'VARCHAR(30)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN xf_ref_typ_name TO _xf_ref_typ_name' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN xf_ref_typ_name DROP DEFAULT';
						END IF;			
					WHEN 'xk_ref_sequence' THEN
						IF rec_column.cube_data_type <> 'NUMERIC(1)' THEN
							EXECUTE 'ALTER TABLE bot.t_service_detail RENAME COLUMN xk_ref_sequence TO _xk_ref_sequence' ;
						ELSE
							EXECUTE 'ALTER TABLE bot.t_service_detail ALTER COLUMN xk_ref_sequence SET DEFAULT ''0''';
						END IF;
					END CASE;			
				WHEN 't_restriction_type_spec_typ' THEN
					CASE rec_column.column_name			
...
DO $BODY$
	...
	BEGIN
		...
		EXECUTE 'CREATE SEQUENCE IF NOT EXISTS bot.sq_svd START WITH 100000';
		...
		EXECUTE 'CREATE TABLE IF NOT EXISTS bot.t_service_detail ()';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS cube_id VARCHAR(16)';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS fk_bot_name VARCHAR(30)';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS fk_typ_name VARCHAR(30)';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS fk_srv_name VARCHAR(30)';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS fk_srv_cube_tsg_db_scr VARCHAR(8) DEFAULT ''D''';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS cube_tsg_atb_ref VARCHAR(8) DEFAULT ''ATB''';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS xf_atb_typ_name VARCHAR(30)';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS xk_atb_name VARCHAR(30)';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS xk_ref_bot_name VARCHAR(30)';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS xk_ref_typ_name VARCHAR(30)';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS xf_ref_typ_name VARCHAR(30)';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD COLUMN IF NOT EXISTS xk_ref_sequence NUMERIC(1) DEFAULT ''0''';
		EXECUTE 'ALTER TABLE bot.t_service_detail ADD CONSTRAINT svd_pk PRIMARY KEY (fk_typ_name, fk_srv_name, fk_srv_cube_tsg_db_scr, xf_atb_typ_name, xk_atb_name, xk_ref_bot_name, xk_ref_typ_name, xf_ref_typ_name, xk_ref_sequence)';
