...
CREATE PROCEDURE bot.get_srv_svd_items (
			p_fk_typ_name IN VARCHAR,
			p_name IN VARCHAR,
			p_cube_tsg_db_scr IN VARCHAR)
LANGUAGE 'plpgsql'
	...
	BEGIN
		...
		OPEN l_cube_cursor FOR
			...
			WHERE fk_typ_name = p_fk_typ_name
			  AND fk_srv_name = p_name
			  AND fk_srv_cube_tsg_db_scr = p_cube_tsg_db_scr
			ORDER BY fk_typ_name, fk_srv_name, fk_srv_cube_tsg_db_scr, cube_tsg_atb_ref, xf_atb_typ_name, xk_atb_name, xk_ref_bot_name, xk_ref_typ_name, xf_ref_typ_name, xk_ref_sequence;
	END;
$BODY$;
	...
	BEGIN
		...
		OPEN l_cube_cursor FOR
			...
			WHERE fk_typ_name = p_fk_typ_name
			  ...
			  AND xk_ref_bot_name = p_xk_ref_bot_name
			  AND xk_ref_typ_name = p_xk_ref_typ_name
			  AND xf_ref_typ_name = p_xf_ref_typ_name
			  AND xk_ref_sequence = p_xk_ref_sequence;
	END;
$BODY$;

CREATE PROCEDURE bot.insert_svd (
			p_fk_bot_name IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
			p_cube_tsg_atb_ref IN VARCHAR,
			p_xf_atb_typ_name IN VARCHAR,
			p_xk_atb_name IN VARCHAR,
			p_xk_ref_bot_name IN VARCHAR,
			p_xk_ref_typ_name IN VARCHAR,
...
AS $BODY$
	...
	BEGIN
		...
		p_fk_srv_name := NULLIF(p_fk_srv_name,'');
		p_fk_srv_cube_tsg_db_scr := NULLIF(p_fk_srv_cube_tsg_db_scr,'');
		p_cube_tsg_atb_ref := NULLIF(p_cube_tsg_atb_ref,'');
		p_xf_atb_typ_name := NULLIF(p_xf_atb_typ_name,'');
		p_xk_atb_name := NULLIF(p_xk_atb_name,'');
		p_xk_ref_bot_name := NULLIF(p_xk_ref_bot_name,'');
		p_xk_ref_typ_name := NULLIF(p_xk_ref_typ_name,'');
		p_xf_ref_typ_name := NULLIF(p_xf_ref_typ_name,'');
		INSERT INTO bot.v_service_detail (
			cube_id,
			fk_bot_name,
			fk_typ_name,
			fk_srv_name,
	...
	EXCEPTION
	WHEN unique_violation THEN
			RAISE EXCEPTION 'Type service_detail already exists';
	END;
$BODY$;

CREATE PROCEDURE bot.update_svd (
			p_fk_bot_name IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
...
AS $BODY$
	...
	BEGIN
		...
		p_xk_ref_typ_name := NULLIF(p_xk_ref_typ_name,'');
		p_xf_ref_typ_name := NULLIF(p_xf_ref_typ_name,'');
		UPDATE bot.v_service_detail SET
			fk_bot_name = p_fk_bot_name,
			cube_tsg_atb_ref = p_cube_tsg_atb_ref
		WHERE fk_typ_name = p_fk_typ_name
		  AND fk_srv_name = p_fk_srv_name
		  AND fk_srv_cube_tsg_db_scr = p_fk_srv_cube_tsg_db_scr
		  AND xf_atb_typ_name = p_xf_atb_typ_name
...
CREATE PROCEDURE bot.delete_svd (
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
			p_xf_atb_typ_name IN VARCHAR,
...
AS $BODY$
	...
	BEGIN
		...
		p_xk_ref_typ_name := NULLIF(p_xk_ref_typ_name,'');
		p_xf_ref_typ_name := NULLIF(p_xf_ref_typ_name,'');
		DELETE FROM bot.v_service_detail
		WHERE fk_typ_name = p_fk_typ_name
		  AND fk_srv_name = p_fk_srv_name
		  AND fk_srv_cube_tsg_db_scr = p_fk_srv_cube_tsg_db_scr
		  AND xf_atb_typ_name = p_xf_atb_typ_name
