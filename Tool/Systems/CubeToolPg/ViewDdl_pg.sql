...
CREATE VIEW itp.v_information_type AS 
	SELECT
		cube_id,
		name
	FROM itp.t_information_type WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW itp.v_information_type_element AS 
	SELECT
		...
		spaces_allowed,
		presentation
	FROM itp.t_information_type_element WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW itp.v_permitted_value AS 
	SELECT
		...
		code,
		prompt
	FROM itp.t_permitted_value WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_business_object_type AS 
	SELECT
		...
		directory,
		api_url
	FROM bot.t_business_object_type WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_type AS 
	SELECT
		...
		icon,
		transferable
	FROM bot.t_type WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_type_specialisation_group AS 
	SELECT
		...
		xf_atb_typ_name,
		xk_atb_name
	FROM bot.t_type_specialisation_group WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_type_specialisation AS 
	SELECT
		...
		xf_tsp_tsg_code,
		xk_tsp_code
	FROM bot.t_type_specialisation WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_attribute AS 
	SELECT
		...
		unchangeable,
		xk_itp_name
	FROM bot.t_attribute WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_derivation AS 
	SELECT
		...
		xk_typ_name,
		xk_typ_name_1
	FROM bot.t_derivation WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_description_attribute AS 
	SELECT
		...
		fk_atb_name,
		text
	FROM bot.t_description_attribute WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_restriction_type_spec_atb AS 
	SELECT
		...
		xf_tsp_tsg_code,
		xk_tsp_code
	FROM bot.t_restriction_type_spec_atb WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_reference AS 
	SELECT
		...
		xk_typ_name,
		xk_typ_name_1
	FROM bot.t_reference WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_description_reference AS 
	SELECT
		...
		fk_ref_typ_name,
		text
	FROM bot.t_description_reference WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_restriction_type_spec_ref AS 
	SELECT
		...
		xf_tsp_tsg_code,
		xk_tsp_code
	FROM bot.t_restriction_type_spec_ref WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_restriction_target_type_spec AS 
	SELECT
		...
		xf_tsp_tsg_code,
		xk_tsp_code
	FROM bot.t_restriction_target_type_spec WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_service AS 
	SELECT
		...
		class,
		accessibility
	FROM bot.t_service WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_service_step AS 
	SELECT
		...
		name,
		script_name
	FROM bot.t_service_step WHERE cube_id <> 'CUBE-NULL';


CREATE VIEW bot.v_service_detail AS 
	SELECT
		cube_id,
		fk_bot_name,
		fk_typ_name,
		...
		xf_ref_typ_name,
		xk_ref_sequence
	FROM bot.t_service_detail WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_restriction_type_spec_typ AS 
	SELECT
		...
		xf_tsp_tsg_code,
		xk_tsp_code
	FROM bot.t_restriction_type_spec_typ WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_json_path AS 
	SELECT
		...
		xk_atb_name,
		xk_typ_name
	FROM bot.t_json_path WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW bot.v_description_type AS 
	SELECT
		...
		fk_typ_name,
		text
	FROM bot.t_description_type WHERE cube_id <> 'CUBE-NULL';
...
AS $BODY$
	BEGIN
		p_srv.cube_id := 'SRV-' || TO_CHAR(NEXTVAL('bot.sq_srv'),'FM000000000000');
...
AS $BODY$
	BEGIN
		p_sst.cube_id := 'SST-' || TO_CHAR(NEXTVAL('bot.sq_sst'),'FM000000000000');
...
CREATE PROCEDURE bot.trg_insert_svd (p_svd bot.v_service_detail)
LANGUAGE plpgsql 
AS $BODY$
	BEGIN
		p_svd.cube_id := 'SVD-' || TO_CHAR(NEXTVAL('bot.sq_svd'),'FM000000000000');
		p_svd.fk_typ_name := COALESCE(p_svd.fk_typ_name,' ');
		p_svd.fk_srv_name := COALESCE(p_svd.fk_srv_name,' ');
		p_svd.fk_srv_cube_tsg_db_scr := COALESCE(p_svd.fk_srv_cube_tsg_db_scr,' ');
		p_svd.xf_atb_typ_name := COALESCE(p_svd.xf_atb_typ_name,' ');
		p_svd.xk_atb_name := COALESCE(p_svd.xk_atb_name,' ');
		p_svd.xk_ref_bot_name := COALESCE(p_svd.xk_ref_bot_name,' ');
		p_svd.xk_ref_typ_name := COALESCE(p_svd.xk_ref_typ_name,' ');
		p_svd.xf_ref_typ_name := COALESCE(p_svd.xf_ref_typ_name,' ');
		p_svd.xk_ref_sequence := COALESCE(p_svd.xk_ref_sequence,0);
		SELECT fk_bot_name
		...
		INSERT INTO bot.t_service_detail (
			cube_id,
			fk_bot_name,
			fk_typ_name,
			fk_srv_name,
...
CREATE PROCEDURE bot.trg_update_svd (p_cube_ctid TID, p_svd_old bot.v_service_detail, p_svd_new bot.v_service_detail)
LANGUAGE plpgsql
AS $BODY$
	BEGIN
		NULL;
...
CREATE PROCEDURE bot.trg_delete_svd (p_cube_ctid TID, p_svd bot.v_service_detail)
LANGUAGE plpgsql 
AS $BODY$
	BEGIN
		DELETE FROM bot.t_service_detail 
		WHERE ctid = p_cube_ctid;
	END;
$BODY$;
...
CREATE FUNCTION bot.trg_svd() RETURNS trigger
LANGUAGE plpgsql 
AS $BODY$
	DECLARE
		l_cube_ctid TID;
		r_svd_new bot.v_service_detail%ROWTYPE;
		r_svd_old bot.v_service_detail%ROWTYPE;
	BEGIN
		IF TG_OP IN ('INSERT','UPDATE') THEN
			IF NEW.fk_bot_name = ' ' THEN
			...
			END IF;
			r_svd_new.fk_bot_name := UPPER(r_svd_new.fk_bot_name);
			IF NEW.fk_typ_name = ' ' THEN
				r_svd_new.fk_typ_name := ' ';
			ELSE
				r_svd_new.fk_typ_name := REPLACE(NEW.fk_typ_name,' ','_');
			END IF;
			r_svd_new.fk_typ_name := UPPER(r_svd_new.fk_typ_name);
			IF NEW.fk_srv_name = ' ' THEN
				r_svd_new.fk_srv_name := ' ';
			ELSE
				r_svd_new.fk_srv_name := REPLACE(NEW.fk_srv_name,' ','_');
			END IF;
			r_svd_new.fk_srv_name := UPPER(r_svd_new.fk_srv_name);
			IF NEW.fk_srv_cube_tsg_db_scr = ' ' THEN
		...
		END IF;
		IF TG_OP IN ('UPDATE','DELETE') THEN
			SELECT ctid INTO l_cube_ctid FROM bot.t_service_detail
			WHERE fk_typ_name = OLD.fk_typ_name
			AND fk_srv_name = OLD.fk_srv_name
			AND fk_srv_cube_tsg_db_scr = OLD.fk_srv_cube_tsg_db_scr
		...
		IF TG_OP = 'INSERT' THEN 
			CALL bot.trg_insert_svd (r_svd_new);
			RETURN NEW;
		ELSIF TG_OP = 'UPDATE' THEN
			CALL bot.trg_update_svd (l_cube_ctid, r_svd_old, r_svd_new);
			RETURN NEW;
		ELSIF TG_OP = 'DELETE' THEN
			CALL bot.trg_delete_svd (l_cube_ctid, r_svd_old);
			RETURN OLD;
		END IF;
		RETURN NULL;
	END;
...
CREATE TRIGGER trg_svd
INSTEAD OF INSERT OR DELETE OR UPDATE ON bot.v_service_detail
FOR EACH ROW
EXECUTE PROCEDURE bot.trg_svd();
...
CREATE VIEW sys.v_system AS 
	SELECT
		...
		password,
		table_prefix
	FROM sys.t_system WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW sys.v_system_bo_type AS 
	SELECT
		...
		fk_sys_name,
		xk_bot_name
	FROM sys.t_system_bo_type WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW fun.v_function AS 
	SELECT
		cube_id,
		name
	FROM fun.t_function WHERE cube_id <> 'CUBE-NULL';
...
CREATE VIEW fun.v_argument AS 
	SELECT
		...
		fk_fun_name,
		name
	FROM fun.t_argument WHERE cube_id <> 'CUBE-NULL';
