...
function OpenDetailPage (p_pageName, p_nodeType, p_objId, p_option) {
	...
	document.getElementById('DetailFrame').src='CubeTool'+p_pageName+'.php?'+encodeURIComponent('{"nodeType":"'+p_nodeType+'","objectId":'+p_objId+l_option+'}');
}
...
function OpenMenu(p_obj) {
	...
	switch (l_type_id) {
 	...
 	case 'TYP_SYS':
		...
		AddMenuItem(g_objMenuList,'execute git_clone_templates','icons/dot.bmp','CubeExecute','E','','SystemGitCloneTemplates',0,'',0);
