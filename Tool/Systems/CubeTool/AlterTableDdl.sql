...
DECLARE
	l_count NUMBER(4);
BEGIN
	SELECT COUNT(1) INTO l_count FROM all_sequences WHERE sequence_owner = 'CUBETOOL' AND sequence_name = 'SQ_SST';
	IF l_count = 0 THEN
		EXECUTE IMMEDIATE 
...
BEGIN
	...
	SELECT COUNT(1) INTO l_count FROM all_tables WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP';
	IF l_count = 0 THEN
		EXECUTE IMMEDIATE
	...
	ELSE
		SELECT COUNT(1) INTO l_count FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND column_name = 'CUBE_ID';
		IF l_count = 0 THEN
			EXECUTE IMMEDIATE
			'ALTER TABLE t_service_step ADD cube_id VARCHAR2(16)';
			DBMS_OUTPUT.PUT_LINE('Column T_SERVICE_STEP.CUBE_ID created');
		END IF;
		SELECT COUNT(1) INTO l_count FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND column_name = 'CUBE_SEQUENCE';
		IF l_count = 0 THEN
			EXECUTE IMMEDIATE
			'ALTER TABLE t_service_step ADD cube_sequence NUMBER(8)';
			DBMS_OUTPUT.PUT_LINE('Column T_SERVICE_STEP.CUBE_SEQUENCE created');
		END IF;
		SELECT COUNT(1) INTO l_count FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND column_name = 'FK_BOT_NAME';
		IF l_count = 0 THEN
			EXECUTE IMMEDIATE
			'ALTER TABLE t_service_step ADD fk_bot_name VARCHAR2(30)';
			DBMS_OUTPUT.PUT_LINE('Column T_SERVICE_STEP.FK_BOT_NAME created');
		END IF;
		SELECT COUNT(1) INTO l_count FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND column_name = 'FK_TYP_NAME';
		IF l_count = 0 THEN
			EXECUTE IMMEDIATE
			'ALTER TABLE t_service_step ADD fk_typ_name VARCHAR2(30)';
			DBMS_OUTPUT.PUT_LINE('Column T_SERVICE_STEP.FK_TYP_NAME created');
		END IF;
		SELECT COUNT(1) INTO l_count FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND column_name = 'FK_SRV_NAME';
		IF l_count = 0 THEN
			EXECUTE IMMEDIATE
			'ALTER TABLE t_service_step ADD fk_srv_name VARCHAR2(30)';
			DBMS_OUTPUT.PUT_LINE('Column T_SERVICE_STEP.FK_SRV_NAME created');
		END IF;
		SELECT COUNT(1) INTO l_count FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND column_name = 'FK_SRV_CUBE_TSG_DB_SCR';
		IF l_count = 0 THEN
			EXECUTE IMMEDIATE
		...
		END IF;
		SELECT COUNT(1) INTO l_count FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND column_name = 'NAME';
		IF l_count = 0 THEN
			EXECUTE IMMEDIATE
			'ALTER TABLE t_service_step ADD name VARCHAR2(30)';
			DBMS_OUTPUT.PUT_LINE('Column T_SERVICE_STEP.NAME created');
		END IF;
		SELECT COUNT(1) INTO l_count FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND column_name = 'SCRIPT_NAME';
		IF l_count = 0 THEN
			EXECUTE IMMEDIATE
			'ALTER TABLE t_service_step ADD script_name VARCHAR2(60)';
			DBMS_OUTPUT.PUT_LINE('Column T_SERVICE_STEP.SCRIPT_NAME created');
		END IF;
		FOR r_key IN (SELECT constraint_name FROM all_constraints WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND constraint_type IN ('P','U','R') ORDER BY constraint_type DESC)
		LOOP
			EXECUTE IMMEDIATE
		...
		END LOOP;
		FOR r_index IN (SELECT index_name FROM all_indexes WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP')
		LOOP
			EXECUTE IMMEDIATE
			'DROP INDEX ' || r_index.index_name;
...
BEGIN
	...
	FOR r_field IN (SELECT column_name,
		...
		DECODE(column_name,
  		...
  		FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP')
	LOOP
		IF r_field.old_domain <> r_field.new_domain THEN
			EXECUTE IMMEDIATE
	...
	'ALTER TABLE t_service_step ADD CONSTRAINT sst_srv_fk
		...
		ON DELETE CASCADE';
	FOR r_field IN (SELECT column_name FROM all_tab_columns WHERE owner = 'CUBETOOL' AND table_name = 'T_SERVICE_STEP' AND column_name NOT IN (
							'CUBE_ID',
							'CUBE_SEQUENCE',
							'FK_BOT_NAME',
							'FK_TYP_NAME',
