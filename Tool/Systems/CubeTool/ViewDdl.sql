...
CREATE OR REPLACE PACKAGE BODY pkg_bot_trg IS
	...
	BEGIN
		p_job.cube_id := 'JOB-' || TO_CHAR(job_seq.NEXTVAL,'FM000000000000');
		SELECT fk_bot_name
	...
	BEGIN
		p_jar.cube_id := 'JAR-' || TO_CHAR(jar_seq.NEXTVAL,'FM000000000000');
		SELECT fk_bot_name
...
BEGIN
	IF INSERTING OR UPDATING THEN
		...
		r_job_new.xk_bot_name := REPLACE(:NEW.xk_bot_name,' ','_');
	END IF;
	IF UPDATING THEN
	...
	IF UPDATING OR DELETING THEN
		...
		r_job_old.xk_bot_name := :OLD.xk_bot_name;
	END IF;
