		...
		if(g_xmlhttp.status == 200) {
			...
			for (i in l_json_array) {
				switch (l_json_array[i].ResultName) {
					...
					case 'LIST_RTS': AddTreeviewChildren(l_json_array[i].Rows,'TYP_RTS','icons/restrict.bmp'); break;
...
function OpenMenu(p_obj) {
	...
	switch (l_type_id) {
 	...
 	case 'TYP_REF':
		...
		AddMenuItem(g_objMenuList, 'add restriction_target_type_spec', 'icons/restrict.bmp','DetailRTS','N','TYP_RTS',0,'N',4);
		var l_json_id = l_json_node_id[l_type_id];
