...
g_xmlhttp.onreadystatechange = function() {
	if(g_xmlhttp.readyState == 4) {
		...
		case "SELECT_SYS":
			...
			document.getElementById("InputDatabase").value=l_values[0];
			document.getElementById("InputSchema").value=l_values[1];
			document.getElementById("InputPassword").value=l_values[2];
			break;
		case "CREATE_SYS":
			...
			document._nodeId = 'TYP_SYS<||>'+document.getElementById("InputName").value;
			if (l_objNode != null) {
				if (l_objNode.firstChild._state == 'O') {
					if (l_argument[1] == null) {
						...
						var l_position = 'B';
						l_objNodePos = parent.TREE.document.getElementById('TYP_SYS<||>'+l_argument[1]);
					}
...
function InitBody() {
	...
	case "D":
		...
		document.getElementById("ButtonCreate").disabled=true;
		performTrans('GetSys'+'<|||>'+document._argument);
		document.getElementById("InputName").readOnly=true;
		break;
	case "N":
		document.getElementById("ButtonUpdate").disabled=true;
...
function CreateSys() {
	...
	performTrans('CreateSys<|||>'+l_parameters);
}
...
function UpdateSys() {
	var l_parameters = 
		...
		document.getElementById("InputSchema").value+'<|>'+
		document.getElementById("InputPassword").value;
	performTrans('UpdateSys<|||>'+l_parameters);
}

function DeleteSys() {
	var l_parameters = 
		document.getElementById("InputName").value;
	performTrans('DeleteSys<|||>'+l_parameters);
}
...
<input id="InputDatabase" type="text" size="30" maxlength="30" onchange="ReplaceSpaces(this);"></input></td></tr>
<tr ><td>Schema</td><td>
<input id="InputSchema" type="text" size="30" maxlength="30" onchange="ReplaceSpaces(this);"></input></td></tr>
<tr ><td>Password</td><td>
<input id="InputPassword" type="text" size="20" maxlength="20" onchange="ReplaceSpaces(this);"></input></td></tr>
<tr><td><br></td></tr>
<tr><td/><td>
