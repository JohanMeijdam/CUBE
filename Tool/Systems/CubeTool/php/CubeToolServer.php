...
case 'GetFunItems':
	...
	$first = True;
	while ($row = oci_fetch_assoc($curs)) {
		if ($first) { $first = False; echo "LIST_ARG";}
		echo "<|||>".$row["FK_FUN_NAME"]."<|>".$row["NAME"];
		echo "<||>".$row["NAME"];
	}
	break;
...
case 'MoveArg':
	...
	list($p_cube_pos_action, $p_fk_fun_name, $p_name, $x_fk_fun_name, $x_name) = explode("<|>", $import[1]);
	...
	END;");
	oci_bind_by_name($stid,":p_cube_pos_action",$p_cube_pos_action);
	oci_bind_by_name($stid,":p_fk_fun_name",$p_fk_fun_name);
	oci_bind_by_name($stid,":p_name",$p_name);
	oci_bind_by_name($stid,":x_fk_fun_name",$x_fk_fun_name);
	oci_bind_by_name($stid,":x_name",$x_name);
...
case 'DeleteArg':
	...
	list($p_fk_fun_name, $p_name) = explode("<|>", $import[1]);
...
case 'GetCubItems':
	...
	while ($row = oci_fetch_assoc($curs)) {
		...
		echo "<|||>".$row["FK_CUB_NAME"]."<|>".$row["HEADER"];
		echo "<||>".$row["HEADER"];
	}
	echo "<||||>";
...
case 'GetCgp':
	...
	list($p_fk_cub_name, $p_header) = explode("<|>", $import[1]);
...
case 'GetCgm':
	...
	echo "SELECT_CGM";
	if ($row = oci_fetch_assoc($curs)) {
		echo "<|||>".$row["INCLUDED_OBJECT_NAMES"];
	}
	break;
...
case 'GetCgmItems':
	...
	while ($row = oci_fetch_assoc($curs)) {
		...
		echo "<|||>".$row["FK_CUB_NAME"]."<|>".$row["FK_CGM_NAME"]."<|>".$row["XK_BOT_NAME"];
		echo "<||>".$row["XK_BOT_NAME"];
	}
	echo "<||||>";
...
case 'GetCgf':
	...
	echo "SELECT_CGF";
	if ($row = oci_fetch_assoc($curs)) {
		echo "<|||>".$row["FK_CUB_NAME"]."<|>".$row["FK_CGM_NAME"]."<|>".$row["DESCRIPTION"]."<|>".$row["TEMPLATE"];
	}
	break;
