		...
		if(g_xmlhttp.status == 200) {
			...
			for (i in l_json_array) {
				switch (l_json_array[i].ResultName) {
					...
					case 'LIST_JOA': AddTreeviewChildren(l_json_array[i].Rows,'TYP_JOA','icons/atb_ref.bmp'); break;
					...
					case 'LIST_CUB': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CUB','icons/cubegen.bmp'); break;
					case 'LIST_CGP': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CGP','icons/desc.bmp'); break;
					...
					case 'LIST_CGM': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CGM','icons/model.bmp'); break;
					...
					case 'LIST_CGO': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CGO','icons/botype.bmp'); break;
					...
					case 'LIST_CGF': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CGF','icons/template.bmp'); break;
					...
					case 'LIST_CTF': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CTF','icons/function.bmp'); break;
...
function InitBody() {
	...
	AddTreeviewNode(l_objBody, 'DIR_CUB', null, 'icons/folder.bmp', 'CubeGen_documentation', 'Y', ' ', null);
	...
	if (p_obj._state == 'O') {
		...
		switch (p_obj.parentNode._type) {
 		...
 		case 'TYP_JSN':
			...
			break;
 		case 'TYP_PAR':
			var l_json_id = JSON.parse(p_obj.parentNode.id)["TYP_PAR"];
			PerformTrans( {Service:"GetParItems",Parameters:{Type:l_json_id}} );
			break;
 		case 'TYP_TSG':
	...
	case 'M':
		if (g_currentParentId == p_obj.parentNode._parentId && g_currentObjIndex >= p_obj.parentNode.parentNode._index || g_currentParentId == p_obj.parentNode.id) {
			...
			case 'TYP_JSN':
				...
				break;
			case 'TYP_STP':
				PerformTrans( {Service:"MoveStp",Parameters:{Option:{CubePosAction:document.body._moveAction},Type:l_json_id,Ref:l_json_id_ref}} );
				break;
			case 'TYP_TSG':
...
function OpenDetailPage (p_code, p_nodeType, p_objId, p_option) {
	...
	document.getElementById('DetailFrame').src='CubeDocuDetail'+p_code+'.php?'+encodeURIComponent('{"nodeType":"'+p_nodeType+'","objectId":'+p_objId+l_option+'}');
}
