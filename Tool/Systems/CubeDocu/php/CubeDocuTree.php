		...
		if(g_xmlhttp.status == 200) {
			...
			for (i in l_json_array) {
				switch (l_json_array[i].ResultName) {
					...
					case 'LIST_CUB': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CUB','icons/cubegen.bmp','CubeGenDocumentation'); break;
					case 'LIST_CGP': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CGP','icons/desc.bmp','CubeGenParagraph'); break;
					...
					case 'LIST_CGM': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CGM','icons/model.bmp','CubeGenExampleModel'); break;
					...
					case 'LIST_CGO': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CGO','icons/botype.bmp','CubeGenExampleObject'); break;
					...
					case 'LIST_CGF': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CGF','icons/template.bmp','CubeGenFunction'); break;
					...
					case 'LIST_CTF': AddTreeviewChildren(l_json_array[i].Rows,'TYP_CTF','icons/function.bmp','CubeGenTemplateFunction'); break;
...
function InitBody() {
	...
	AddTreeviewNode(l_objBody, 'DIR_CUB', null, 'icons/folder.bmp', null, 'CubeGen_documentation', 'Y', ' ', null);
	...
	if (p_obj._state == 'O') {
		...
		switch (p_obj.parentNode._type) {
 		...
 		case 'TYP_JSN':
			...
			break;
 		case 'DIR_CUB':
			PerformTrans( {Service:"GetDirCubItems"} );
			break;
 		case 'TYP_CUB':
			var l_json_id = JSON.parse(p_obj.parentNode.id)["TYP_CUB"];
			PerformTrans( {Service:"GetCubItems",Parameters:{Type:l_json_id}} );
			break;
 		case 'TYP_CGM':
			var l_json_id = JSON.parse(p_obj.parentNode.id)["TYP_CGM"];
			PerformTrans( {Service:"GetCgmItems",Parameters:{Type:l_json_id}} );
			break;
 		case 'DIR_SYS':
	...
	case 'M':
		if (g_currentParentId == p_obj.parentNode._parentId && g_currentObjIndex >= p_obj.parentNode.parentNode._index || g_currentParentId == p_obj.parentNode.id) {
			...
			case 'TYP_JSN':
				...
				break;
			case 'TYP_CGP':
				PerformTrans( {Service:"MoveCgp",Parameters:{Option:{CubePosAction:document.body._moveAction},Type:l_json_id,Ref:l_json_id_ref}} );
				break;
			case 'TYP_CGM':
				PerformTrans( {Service:"MoveCgm",Parameters:{Option:{CubePosAction:document.body._moveAction},Type:l_json_id,Ref:l_json_id_ref}} );
				break;
			case 'TYP_CGO':
				PerformTrans( {Service:"MoveCgo",Parameters:{Option:{CubePosAction:document.body._moveAction},Type:l_json_id,Ref:l_json_id_ref}} );
				break;
			case 'TYP_CGF':
				PerformTrans( {Service:"MoveCgf",Parameters:{Option:{CubePosAction:document.body._moveAction},Type:l_json_id,Ref:l_json_id_ref}} );
				break;
			case 'TYP_SBT':
...
function OpenDetailPage (p_name, p_nodeType, p_objId, p_option) {
	...
	document.getElementById('DetailFrame').src='CubeDocuMaintain'+p_name+'.php?'+encodeURIComponent('{"nodeType":"'+p_nodeType+'","objectId":'+p_objId+l_option+'}');
}
...
function OpenMenu(p_obj) {
	...
	switch (l_type_id) {
 	...
 	case 'TYP_JSN':
		...
		break;
 	case 'DIR_CUB':
		AddMenuItem(g_objMenuList, 'add cube_gen_documentation', 'icons/cubegen.bmp','DetailCUB','N','TYP_CUB','CubeGenDocumentation',0,'N',2);
		break;
 	case 'TYP_CUB':
		AddMenuItem(g_objMenuList, 'add cube_gen_paragraph', 'icons/desc.bmp','CubeAdd','N','TYP_CGP','CubeGenParagraph',0,'N',2);
		AddMenuItem(g_objMenuList, 'add cube_gen_example_model', 'icons/model.bmp','CubeAdd','N','TYP_CGM','CubeGenExampleModel',0,'N',3);
		AddMenuItem(g_objMenuList, 'add cube_gen_template_function', 'icons/function.bmp','DetailCTF','N','TYP_CTF','CubeGenTemplateFunction',0,'N',4);
		break;
 	case 'TYP_CGP':
		if (l_childCount > 1) {
 	...
 	case 'TYP_CGM':
		...
		AddMenuItem(g_objMenuList, 'add cube_gen_example_object', 'icons/botype.bmp','CubeAdd','N','TYP_CGO','CubeGenExampleObject',0,'N',2);
		AddMenuItem(g_objMenuList, 'add cube_gen_function', 'icons/template.bmp','CubeAdd','N','TYP_CGF','CubeGenFunction',0,'N',3);
		break;
 	case 'TYP_CGO':
		if (l_childCount > 1) {
