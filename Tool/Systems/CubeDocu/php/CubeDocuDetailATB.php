...
g_xmlhttp.onreadystatechange = function() {
	if(g_xmlhttp.readyState == 4) {
		...
		case "SELECT_ATB":
			...
			document.getElementById("InputPrimaryKey").value=l_values[1];
			document.getElementById("InputCodeDisplayKey").value=l_values[2];
			document.getElementById("InputCodeForeignKey").value=l_values[3];
			document.getElementById("InputFlagHidden").value=l_values[4];
			document.getElementById("InputDefaultValue").value=l_values[5];
			document.getElementById("InputUnchangeable").value=l_values[6];
			document.getElementById("InputXkItpName").value=l_values[7];
			break;
		case "CREATE_ATB":
			...
			document._nodeId = 'TYP_ATB<||>'+document.getElementById("InputFkTypName").value+'<|>'+document.getElementById("InputName").value;
			if (l_objNode != null) {
				if (l_objNode.firstChild._state == 'O') {
					var l_position = g_option[0];
					l_objNodePos = parent.TREE.document.getElementById('TYP_ATB<||>'+g_option[1]);
		...
		case "DELETE_ATB":
			...
			break;
		case "LIST_ITP":
			OpenListBox(l_argument,'inftype','InformationType','Y');
			break;
...
function InitBody() {
	...
	case "D":
		...
		document.getElementById("ButtonCreate").disabled=true;
		performTrans('GetAtb'+'<|||>'+document._argument);
		document.getElementById("InputFkBotName").readOnly=true;
		document.getElementById("InputFkTypName").readOnly=true;
		document.getElementById("InputName").readOnly=true;
		break;
...
function CreateAtb() {
	var l_parameters = 
		...
		document.getElementById("InputCodeForeignKey").value+'<|>'+
		document.getElementById("InputFlagHidden").value+'<|>'+
	...
	if (g_option[0] == 'F' || g_option[0] == 'L') {
		performTrans('CreateAtb<|||>'+g_option[0]+'<|>'+l_parameters);
	} else {
		performTrans('CreateAtb<|||>'+g_option[0]+'<|>'+l_parameters+'<|>'+g_option[1]);
	}
}
...
function UpdateAtb() {
	var l_parameters = 
		...
		document.getElementById("InputUnchangeable").value+'<|>'+
		document.getElementById("InputXkItpName").value;
	performTrans('UpdateAtb<|||>'+l_parameters);
}
...
function DeleteAtb() {
	var l_parameters = 
		document.getElementById("InputFkTypName").value+'<|>'+
		document.getElementById("InputName").value;
	performTrans('DeleteAtb<|||>'+l_parameters);
}
...
function UpdateForeignKey(p_obj) {
	...
	case "Ref001":
		if (l_obj_option.value == '') {
			...
			document.getElementById("InputXkItpName").value = l_values[0];
		}
		break;
	default:
