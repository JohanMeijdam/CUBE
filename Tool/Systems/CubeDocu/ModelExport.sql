...
DECLARE
	...
	BEGIN
		FOR r_cgf IN (
			...
			ORDER BY cube_sequence )
		LOOP
			DBMS_OUTPUT.PUT_LINE (ftabs || '=CUBE_GEN_FUNCTION[' || r_cgf.cube_id || ']:' || fenperc(r_cgf.id) || '|' || fenperc(r_cgf.header) || '|' || fenperc(r_cgf.description) || '|' || fenperc(r_cgf.template) || ';');
				l_level := l_level + 1;
				l_level := l_level - 1;
		END LOOP;
	END;
	...
	BEGIN
		FOR r_cgm IN (
			...
			ORDER BY cube_sequence )
		LOOP
			DBMS_OUTPUT.PUT_LINE (ftabs || '+CUBE_GEN_EXAMPLE_MODEL[' || r_cgm.cube_id || ']:' || fenperc(r_cgm.id) || '|' || fenperc(r_cgm.header) || '|' || fenperc(r_cgm.included_object_names) || '|' || fenperc(r_cgm.description) || ';');
				l_level := l_level + 1;
				report_cgo (r_cgm);
				report_cgf (r_cgm);
				l_level := l_level - 1;
			DBMS_OUTPUT.PUT_LINE (ftabs || '-CUBE_GEN_EXAMPLE_MODEL:' || r_cgm.id || ';');
		END LOOP;
	END;
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE ('			=PROPERTY:0|Id|'||REPLACE('Technical%20identifier.','%20',' ')||';');
	DBMS_OUTPUT.PUT_LINE ('			=PROPERTY:1|Header|'||REPLACE('Text%20used%20as%20header%20and%20used%20in%20the%20index.','%20',' ')||';');
	DBMS_OUTPUT.PUT_LINE ('			=PROPERTY:2|IncludedObjectNames|'||REPLACE('The%20names%20of%20types%20that%20are%20included%20in%20the%20view%20of%20the%20business%20object%20model.','%20',' ')||';');
	DBMS_OUTPUT.PUT_LINE ('			=PROPERTY:3|Description|;');
	DBMS_OUTPUT.PUT_LINE ('			+META_TYPE:CUBE_GEN_EXAMPLE_OBJECT|'||REPLACE('A%20reference%20to%20a%20business%20object%20that%20is%20used%20in%20the%20examples.','%20',' ')||';');
