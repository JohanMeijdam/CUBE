			...
			switch ($NodeString[$_[0]]) {
				...
				case "SERVICE_ARGUMENT" {
					...
					while (1) {
						...
						print IMPORT "	VALUES ($Sequence, '$_[1]', '$_[2]', '$_[3]', '$_[4]', '".ReplX(GetXkey($j,'SERVICE_ARGUMENT_ATTRIBUTE','TYPE',001))."', '".ReplX(GetXkey($j,'SERVICE_ARGUMENT_ATTRIBUTE','ATTRIBUTE',001))."');\n";
						print IMPORT "\n";
						$j = $NodeNext[$j];
					}
				}
