...
g_xmlhttp.onreadystatechange = function() {
	if(g_xmlhttp.readyState == 4) {
		...
		case "SELECT_ITE":
			...
			document.getElementById("InputSuffix").value=l_values[0];
			document.getElementById("InputDomain").value=l_values[1];
			document.getElementById("InputLength").value=l_values[2];
			document.getElementById("InputDecimals").value=l_values[3];
		...
		case "CREATE_ITE":
			...
			document._nodeId = 'TYP_ITE<||>'+document.getElementById("InputFkItpName").value+'<|>'+document.getElementById("InputSequence").value;
			if (l_objNode != null) {
				if (l_objNode.firstChild._state == 'O') {
					if (l_argument[1] == null) {
						...
						var l_position = 'B';
						l_objNodePos = parent.TREE.document.getElementById('TYP_ITE<||>'+l_argument[1]);
					}
...
function InitBody() {
	...
	case "D":
		...
		document.getElementById("ButtonCreate").disabled=true;
		performTrans('GetIte'+'<|||>'+document._argument);
		document.getElementById("InputFkItpName").readOnly=true;
		document.getElementById("InputSequence").readOnly=true;
		break;
	case "N":
...
function CreateIte() {
	var l_parameters = 
		...
		document.getElementById("InputDecimals").value+'<|>'+
...
function DeleteIte() {
	var l_parameters = 
		document.getElementById("InputFkItpName").value+'<|>'+
		document.getElementById("InputSequence").value;
	performTrans('DeleteIte<|||>'+l_parameters);
}
