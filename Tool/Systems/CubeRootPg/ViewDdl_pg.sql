	...
	BEGIN
		...
		FOR rec_view IN 
			...
			WHERE schemaname = 'itp'
			  AND viewowner = '<<2>>'
			  AND COALESCE(SUBSTR(viewname,1,INSTR(viewname,'_',3)),' ') NOT IN ('V_CUBE_')
		LOOP
...
AS $BODY$
	BEGIN
		p_itp.cube_id := 'CUBE-ITP-' || TO_CHAR(NEXTVAL('itp.sq_itp'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_ite.cube_id := 'CUBE-ITE-' || TO_CHAR(NEXTVAL('itp.sq_ite'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_val.cube_id := 'CUBE-VAL-' || TO_CHAR(NEXTVAL('itp.sq_val'),'FM0000000');
...
CREATE VIEW bot.v_service_step AS 
	SELECT
		cube_id,
		cube_sequence,
		fk_bot_name,
		...
		name,
		script_name
	FROM bot.t_service_step;


CREATE VIEW bot.v_service_argument AS 
	SELECT
		cube_id,
		cube_sequence,
		fk_bot_name,
...
AS $BODY$
	BEGIN
		p_bot.cube_id := 'CUBE-BOT-' || TO_CHAR(NEXTVAL('bot.sq_bot'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_typ.cube_id := 'CUBE-TYP-' || TO_CHAR(NEXTVAL('bot.sq_typ'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_tsg.cube_id := 'CUBE-TSG-' || TO_CHAR(NEXTVAL('bot.sq_tsg'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_tsp.cube_id := 'CUBE-TSP-' || TO_CHAR(NEXTVAL('bot.sq_tsp'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_atb.cube_id := 'CUBE-ATB-' || TO_CHAR(NEXTVAL('bot.sq_atb'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_der.cube_id := 'CUBE-DER-' || TO_CHAR(NEXTVAL('bot.sq_der'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_dca.cube_id := 'CUBE-DCA-' || TO_CHAR(NEXTVAL('bot.sq_dca'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_rta.cube_id := 'CUBE-RTA-' || TO_CHAR(NEXTVAL('bot.sq_rta'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_ref.cube_id := 'CUBE-REF-' || TO_CHAR(NEXTVAL('bot.sq_ref'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_dcr.cube_id := 'CUBE-DCR-' || TO_CHAR(NEXTVAL('bot.sq_dcr'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_rtr.cube_id := 'CUBE-RTR-' || TO_CHAR(NEXTVAL('bot.sq_rtr'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_rts.cube_id := 'CUBE-RTS-' || TO_CHAR(NEXTVAL('bot.sq_rts'),'FM0000000');
...
CREATE PROCEDURE bot.trg_insert_srv (p_srv bot.v_service)
LANGUAGE plpgsql 
AS $BODY$
	BEGIN
		p_srv.cube_id := 'CUBE-SRV-' || TO_CHAR(NEXTVAL('bot.sq_srv'),'FM0000000');
		p_srv.fk_bot_name := COALESCE(p_srv.fk_bot_name,' ');
		p_srv.fk_typ_name := COALESCE(p_srv.fk_typ_name,' ');
		p_srv.name := COALESCE(p_srv.name,' ');
		p_srv.cube_tsg_db_scr := COALESCE(p_srv.cube_tsg_db_scr,' ');
		SELECT fk_bot_name
		  INTO p_srv.fk_bot_name
		FROM bot.t_type
		WHERE name = p_srv.fk_typ_name;
		INSERT INTO bot.t_service (
			cube_id,
			cube_sequence,
			fk_bot_name,
			fk_typ_name,
...
CREATE PROCEDURE bot.trg_update_srv (p_cube_ctid TID, p_srv_old bot.v_service, p_srv_new bot.v_service)
LANGUAGE plpgsql
AS $BODY$
	BEGIN
		UPDATE bot.t_service SET 
...
CREATE PROCEDURE bot.trg_delete_srv (p_cube_ctid TID, p_srv bot.v_service)
LANGUAGE plpgsql 
AS $BODY$
	BEGIN
		DELETE FROM bot.t_service 
		WHERE ctid = p_cube_ctid;
	END;
$BODY$;

CREATE PROCEDURE bot.trg_insert_sst (p_sst bot.v_service_step)
LANGUAGE plpgsql 
AS $BODY$
	BEGIN
		p_sst.cube_id := 'CUBE-SST-' || TO_CHAR(NEXTVAL('bot.sq_sst'),'FM0000000');
		p_sst.fk_bot_name := COALESCE(p_sst.fk_bot_name,' ');
		p_sst.fk_typ_name := COALESCE(p_sst.fk_typ_name,' ');
		p_sst.fk_srv_name := COALESCE(p_sst.fk_srv_name,' ');
		p_sst.fk_srv_cube_tsg_db_scr := COALESCE(p_sst.fk_srv_cube_tsg_db_scr,' ');
		p_sst.name := COALESCE(p_sst.name,' ');
		SELECT fk_bot_name
		...
		INSERT INTO bot.t_service_step (
			cube_id,
			cube_sequence,
			fk_bot_name,
			fk_typ_name,
...
CREATE PROCEDURE bot.trg_update_sst (p_cube_ctid TID, p_sst_old bot.v_service_step, p_sst_new bot.v_service_step)
LANGUAGE plpgsql
AS $BODY$
	BEGIN
		UPDATE bot.t_service_step SET 
...
CREATE PROCEDURE bot.trg_delete_sst (p_cube_ctid TID, p_sst bot.v_service_step)
LANGUAGE plpgsql 
AS $BODY$
	BEGIN
		DELETE FROM bot.t_service_step 
		WHERE ctid = p_cube_ctid;
	END;
$BODY$;

CREATE PROCEDURE bot.trg_insert_sva (p_sva bot.v_service_argument)
LANGUAGE plpgsql 
AS $BODY$
	BEGIN
		p_sva.cube_id := 'CUBE-SVA-' || TO_CHAR(NEXTVAL('bot.sq_sva'),'FM0000000');
		p_sva.fk_bot_name := COALESCE(p_sva.fk_bot_name,' ');
		p_sva.fk_typ_name := COALESCE(p_sva.fk_typ_name,' ');
		p_sva.fk_srv_name := COALESCE(p_sva.fk_srv_name,' ');
		p_sva.fk_srv_cube_tsg_db_scr := COALESCE(p_sva.fk_srv_cube_tsg_db_scr,' ');
		p_sva.option_name := COALESCE(p_sva.option_name,' ');
		p_sva.xk_itp_name := COALESCE(p_sva.xk_itp_name,' ');
		p_sva.xf_atb_typ_name := COALESCE(p_sva.xf_atb_typ_name,' ');
		p_sva.xk_atb_name := COALESCE(p_sva.xk_atb_name,' ');
		p_sva.xk_ref_bot_name := COALESCE(p_sva.xk_ref_bot_name,' ');
		p_sva.xk_ref_typ_name := COALESCE(p_sva.xk_ref_typ_name,' ');
		p_sva.xf_ref_typ_name := COALESCE(p_sva.xf_ref_typ_name,' ');
		p_sva.xk_ref_sequence := COALESCE(p_sva.xk_ref_sequence,0);
		SELECT fk_bot_name
		...
		INSERT INTO bot.t_service_argument (
			cube_id,
			cube_sequence,
			fk_bot_name,
			fk_typ_name,
...
CREATE PROCEDURE bot.trg_update_sva (p_cube_ctid TID, p_sva_old bot.v_service_argument, p_sva_new bot.v_service_argument)
LANGUAGE plpgsql
AS $BODY$
	BEGIN
		UPDATE bot.t_service_argument SET 
...
CREATE PROCEDURE bot.trg_delete_sva (p_cube_ctid TID, p_sva bot.v_service_argument)
LANGUAGE plpgsql 
AS $BODY$
	BEGIN
		DELETE FROM bot.t_service_argument 
		WHERE ctid = p_cube_ctid;
	END;
$BODY$;
...
AS $BODY$
	BEGIN
		p_rtt.cube_id := 'CUBE-RTT-' || TO_CHAR(NEXTVAL('bot.sq_rtt'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_jsn.cube_id := 'CUBE-JSN-' || TO_CHAR(NEXTVAL('bot.sq_jsn'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_dct.cube_id := 'CUBE-DCT-' || TO_CHAR(NEXTVAL('bot.sq_dct'),'FM0000000');
...
CREATE FUNCTION bot.trg_srv() RETURNS trigger
LANGUAGE plpgsql 
AS $BODY$
	DECLARE
		l_cube_ctid TID;
		r_srv_new bot.v_service%ROWTYPE;
		r_srv_old bot.v_service%ROWTYPE;
	BEGIN
		IF TG_OP IN ('INSERT','UPDATE') THEN
			...
			END IF;
			r_srv_new.fk_bot_name := UPPER(r_srv_new.fk_bot_name);
			IF NEW.fk_typ_name = ' ' THEN
				r_srv_new.fk_typ_name := ' ';
			ELSE
				r_srv_new.fk_typ_name := REPLACE(NEW.fk_typ_name,' ','_');
			END IF;
			r_srv_new.fk_typ_name := UPPER(r_srv_new.fk_typ_name);
			IF NEW.name = ' ' THEN
				r_srv_new.name := ' ';
			ELSE
				r_srv_new.name := REPLACE(NEW.name,' ','_');
			END IF;
			r_srv_new.name := UPPER(r_srv_new.name);
			...
			END IF;
			r_srv_new.class := UPPER(r_srv_new.class);
			IF NEW.accessibility = ' ' THEN
				r_srv_new.accessibility := ' ';
			ELSE
				r_srv_new.accessibility := REPLACE(NEW.accessibility,' ','_');
			END IF;
			r_srv_new.accessibility := UPPER(r_srv_new.accessibility);
		END IF;
		IF TG_OP IN ('UPDATE') THEN
			r_srv_new.cube_id := OLD.cube_id;
		END IF;
		IF TG_OP IN ('UPDATE','DELETE') THEN
			SELECT ctid INTO l_cube_ctid FROM bot.t_service
			WHERE fk_typ_name = OLD.fk_typ_name
			AND name = OLD.name
		...
		IF TG_OP = 'INSERT' THEN 
			CALL bot.trg_insert_srv (r_srv_new);
			RETURN NEW;
		ELSIF TG_OP = 'UPDATE' THEN
			CALL bot.trg_update_srv (l_cube_ctid, r_srv_old, r_srv_new);
			RETURN NEW;
		ELSIF TG_OP = 'DELETE' THEN
			CALL bot.trg_delete_srv (l_cube_ctid, r_srv_old);
			RETURN OLD;
		END IF;
		RETURN NULL;
	END;
...
CREATE TRIGGER trg_srv
INSTEAD OF INSERT OR DELETE OR UPDATE ON bot.v_service
FOR EACH ROW
EXECUTE PROCEDURE bot.trg_srv();

CREATE FUNCTION bot.trg_sst() RETURNS trigger
LANGUAGE plpgsql 
AS $BODY$
	DECLARE
		l_cube_ctid TID;
		r_sst_new bot.v_service_step%ROWTYPE;
		r_sst_old bot.v_service_step%ROWTYPE;
	BEGIN
		IF TG_OP IN ('INSERT','UPDATE') THEN
			...
			END IF;
			r_sst_new.name := UPPER(r_sst_new.name);
		...
		END IF;
		IF TG_OP IN ('UPDATE','DELETE') THEN
			SELECT ctid INTO l_cube_ctid FROM bot.t_service_step
			WHERE fk_typ_name = OLD.fk_typ_name
		...
		IF TG_OP = 'INSERT' THEN 
			CALL bot.trg_insert_sst (r_sst_new);
			RETURN NEW;
		ELSIF TG_OP = 'UPDATE' THEN
			CALL bot.trg_update_sst (l_cube_ctid, r_sst_old, r_sst_new);
			RETURN NEW;
		ELSIF TG_OP = 'DELETE' THEN
			CALL bot.trg_delete_sst (l_cube_ctid, r_sst_old);
			RETURN OLD;
		END IF;
		RETURN NULL;
	END;
...
CREATE TRIGGER trg_sst
INSTEAD OF INSERT OR DELETE OR UPDATE ON bot.v_service_step
FOR EACH ROW
EXECUTE PROCEDURE bot.trg_sst();

CREATE FUNCTION bot.trg_sva() RETURNS trigger
LANGUAGE plpgsql 
AS $BODY$
	DECLARE
		l_cube_ctid TID;
		r_sva_new bot.v_service_argument%ROWTYPE;
		r_sva_old bot.v_service_argument%ROWTYPE;
	BEGIN
		IF TG_OP IN ('INSERT','UPDATE') THEN
			...
			END IF;
			r_sva_new.fk_bot_name := UPPER(r_sva_new.fk_bot_name);
			IF NEW.fk_typ_name = ' ' THEN
				r_sva_new.fk_typ_name := ' ';
			ELSE
				r_sva_new.fk_typ_name := REPLACE(NEW.fk_typ_name,' ','_');
			END IF;
			r_sva_new.fk_typ_name := UPPER(r_sva_new.fk_typ_name);
			IF NEW.fk_srv_name = ' ' THEN
				r_sva_new.fk_srv_name := ' ';
			ELSE
				r_sva_new.fk_srv_name := REPLACE(NEW.fk_srv_name,' ','_');
			END IF;
			r_sva_new.fk_srv_name := UPPER(r_sva_new.fk_srv_name);
			IF NEW.fk_srv_cube_tsg_db_scr = ' ' THEN
		...
		END IF;
		IF TG_OP IN ('UPDATE','DELETE') THEN
			SELECT ctid INTO l_cube_ctid FROM bot.t_service_argument
			WHERE fk_typ_name = OLD.fk_typ_name
			AND fk_srv_name = OLD.fk_srv_name
			AND fk_srv_cube_tsg_db_scr = OLD.fk_srv_cube_tsg_db_scr
		...
		IF TG_OP = 'INSERT' THEN 
			CALL bot.trg_insert_sva (r_sva_new);
			RETURN NEW;
		ELSIF TG_OP = 'UPDATE' THEN
			CALL bot.trg_update_sva (l_cube_ctid, r_sva_old, r_sva_new);
			RETURN NEW;
		ELSIF TG_OP = 'DELETE' THEN
			CALL bot.trg_delete_sva (l_cube_ctid, r_sva_old);
			RETURN OLD;
		END IF;
		RETURN NULL;
	END;
...
CREATE TRIGGER trg_sva
INSTEAD OF INSERT OR DELETE OR UPDATE ON bot.v_service_argument
FOR EACH ROW
EXECUTE PROCEDURE bot.trg_sva();
...
AS $BODY$
	BEGIN
		p_sys.cube_id := 'CUBE-SYS-' || TO_CHAR(NEXTVAL('sys.sq_sys'),'FM0000000');
...
AS $BODY$
	BEGIN
		p_sbt.cube_id := 'CUBE-SBT-' || TO_CHAR(NEXTVAL('sys.sq_sbt'),'FM0000000');
