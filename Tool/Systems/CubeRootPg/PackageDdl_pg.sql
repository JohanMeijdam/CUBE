...
CREATE PROCEDURE bot.get_typ_srv_items (
			p_name IN VARCHAR)
LANGUAGE 'plpgsql'
AS $BODY$
	DECLARE
...
CREATE PROCEDURE bot.get_ref_for_typ_list (
			p_cube_scope_level IN NUMERIC,
			x_fk_typ_name IN VARCHAR)
LANGUAGE 'plpgsql'
AS $BODY$
...
AS $BODY$
	...
	BEGIN
		...
		p_type_prefix := NULLIF(p_type_prefix,'');
		p_xk_bot_name := NULLIF(p_xk_bot_name,'');
		p_xk_typ_name := NULLIF(p_xk_typ_name,'');
		p_xk_typ_name_1 := NULLIF(p_xk_typ_name_1,'');
		x_fk_typ_name := NULLIF(x_fk_typ_name,'');
...
CREATE PROCEDURE bot.get_srv (
			p_fk_typ_name IN VARCHAR,
			p_name IN VARCHAR,
			p_cube_tsg_db_scr IN VARCHAR)
LANGUAGE 'plpgsql'
AS $BODY$
	DECLARE
		l_cube_cursor REFCURSOR := 'cube_cursor';
	BEGIN
		p_fk_typ_name := NULLIF(p_fk_typ_name,'');
		p_name := NULLIF(p_name,'');
		p_cube_tsg_db_scr := NULLIF(p_cube_tsg_db_scr,'');
		OPEN l_cube_cursor FOR
			SELECT
			  fk_bot_name,
			  class,
...
CREATE PROCEDURE bot.get_srv_fkey (
			p_fk_typ_name IN VARCHAR,
			p_name IN VARCHAR,
			p_cube_tsg_db_scr IN VARCHAR)
LANGUAGE 'plpgsql'
...
CREATE PROCEDURE bot.get_srv_sst_items (
			p_fk_typ_name IN VARCHAR,
			p_name IN VARCHAR,
			p_cube_tsg_db_scr IN VARCHAR)
LANGUAGE 'plpgsql'
...
CREATE PROCEDURE bot.get_srv_sva_items (
			p_fk_typ_name IN VARCHAR,
			p_name IN VARCHAR,
			p_cube_tsg_db_scr IN VARCHAR)
LANGUAGE 'plpgsql'
...
CREATE PROCEDURE bot.determine_position_srv (
			p_cube_sequence INOUT NUMERIC,
			p_cube_pos_action IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_name IN VARCHAR,
...
AS $BODY$
	...
	BEGIN
		...
		x_fk_typ_name := NULLIF(x_fk_typ_name,'');
		x_name := NULLIF(x_name,'');
		x_cube_tsg_db_scr := NULLIF(x_cube_tsg_db_scr,'');
		-- A=After B=Before F=First L=Last
		IF COALESCE(p_cube_pos_action, ' ') NOT IN  ('A', 'B', 'F', 'L') THEN
			RAISE EXCEPTION 'Invalid position action: %', p_cube_pos_action;
		END IF;
		CALL bot.determine_position_srv  (l_cube_sequence, p_cube_pos_action, x_fk_typ_name, x_name, x_cube_tsg_db_scr);
		UPDATE bot.v_service SET
			cube_sequence = l_cube_sequence
		WHERE fk_typ_name = p_fk_typ_name
		  AND name = p_name
		  AND cube_tsg_db_scr = p_cube_tsg_db_scr;
		IF NOT FOUND THEN
			RAISE EXCEPTION 'Type service not found';
		END IF;
	END;
$BODY$;

CREATE PROCEDURE bot.insert_srv (
			p_cube_pos_action IN VARCHAR,
			p_fk_bot_name IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_name IN VARCHAR,
...
AS $BODY$
	...
	BEGIN
		...
		p_name := NULLIF(p_name,'');
		p_cube_tsg_db_scr := NULLIF(p_cube_tsg_db_scr,'');
		p_class := NULLIF(p_class,'');
		p_accessibility := NULLIF(p_accessibility,'');
		x_fk_typ_name := NULLIF(x_fk_typ_name,'');
		x_name := NULLIF(x_name,'');
		x_cube_tsg_db_scr := NULLIF(x_cube_tsg_db_scr,'');
		-- A=After B=Before F=First L=Last
	...
	EXCEPTION
	WHEN unique_violation THEN
			RAISE EXCEPTION 'Type service already exists';
	END;
$BODY$;

CREATE PROCEDURE bot.update_srv (
			p_fk_bot_name IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_name IN VARCHAR,
			p_cube_tsg_db_scr IN VARCHAR,
...
CREATE PROCEDURE bot.delete_srv (
			p_fk_typ_name IN VARCHAR,
			p_name IN VARCHAR,
			p_cube_tsg_db_scr IN VARCHAR)
LANGUAGE 'plpgsql'
...
CREATE PROCEDURE bot.get_sst (
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
			p_name IN VARCHAR)
LANGUAGE 'plpgsql'
AS $BODY$
	DECLARE
	...
	BEGIN
		p_fk_typ_name := NULLIF(p_fk_typ_name,'');
		p_fk_srv_name := NULLIF(p_fk_srv_name,'');
		p_fk_srv_cube_tsg_db_scr := NULLIF(p_fk_srv_cube_tsg_db_scr,'');
		p_name := NULLIF(p_name,'');
		OPEN l_cube_cursor FOR
			SELECT
			  fk_bot_name,
...
CREATE PROCEDURE bot.determine_position_sst (
			p_cube_sequence INOUT NUMERIC,
			p_cube_pos_action IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
...
CREATE PROCEDURE bot.move_sst (
			...
			p_name IN VARCHAR,
			x_fk_typ_name IN VARCHAR,
			x_fk_srv_name IN VARCHAR,
			x_fk_srv_cube_tsg_db_scr IN VARCHAR,
			x_name IN VARCHAR)
LANGUAGE 'plpgsql'
AS $BODY$
	DECLARE
	...
	BEGIN
		...
		p_name := NULLIF(p_name,'');
		x_fk_typ_name := NULLIF(x_fk_typ_name,'');
		x_fk_srv_name := NULLIF(x_fk_srv_name,'');
		x_fk_srv_cube_tsg_db_scr := NULLIF(x_fk_srv_cube_tsg_db_scr,'');
		x_name := NULLIF(x_name,'');
		-- A=After B=Before F=First L=Last
		IF COALESCE(p_cube_pos_action, ' ') NOT IN  ('A', 'B', 'F', 'L') THEN
			RAISE EXCEPTION 'Invalid position action: %', p_cube_pos_action;
		END IF;
		CALL bot.determine_position_sst  (l_cube_sequence, p_cube_pos_action, x_fk_typ_name, x_fk_srv_name, x_fk_srv_cube_tsg_db_scr, x_name);
		UPDATE bot.v_service_step SET
			cube_sequence = l_cube_sequence
		WHERE fk_typ_name = p_fk_typ_name
		  AND fk_srv_name = p_fk_srv_name
		  ...
		  AND name = p_name;
		IF NOT FOUND THEN
			RAISE EXCEPTION 'Type service_step not found';
		END IF;
	END;
$BODY$;

CREATE PROCEDURE bot.insert_sst (
			p_cube_pos_action IN VARCHAR,
			p_fk_bot_name IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
...
AS $BODY$
	...
	BEGIN
		...
		p_fk_srv_cube_tsg_db_scr := NULLIF(p_fk_srv_cube_tsg_db_scr,'');
		p_name := NULLIF(p_name,'');
		p_script_name := NULLIF(p_script_name,'');
		x_fk_typ_name := NULLIF(x_fk_typ_name,'');
		x_fk_srv_name := NULLIF(x_fk_srv_name,'');
		x_fk_srv_cube_tsg_db_scr := NULLIF(x_fk_srv_cube_tsg_db_scr,'');
		x_name := NULLIF(x_name,'');
		...
		END IF;
		CALL bot.determine_position_sst  (l_cube_sequence, p_cube_pos_action, x_fk_typ_name, x_fk_srv_name, x_fk_srv_cube_tsg_db_scr, x_name);
		INSERT INTO bot.v_service_step (
			cube_id,
			cube_sequence,
			fk_bot_name,
			fk_typ_name,
	...
	EXCEPTION
	WHEN unique_violation THEN
			RAISE EXCEPTION 'Type service_step already exists';
	END;
$BODY$;

CREATE PROCEDURE bot.update_sst (
			p_fk_bot_name IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
...
CREATE PROCEDURE bot.delete_sst (
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
			p_name IN VARCHAR)
...
CREATE PROCEDURE bot.get_sva (
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
			p_option_name IN VARCHAR,
			p_xf_atb_typ_name IN VARCHAR,
			p_xk_atb_name IN VARCHAR)
LANGUAGE 'plpgsql'
AS $BODY$
	...
	BEGIN
		...
		p_fk_srv_name := NULLIF(p_fk_srv_name,'');
		p_fk_srv_cube_tsg_db_scr := NULLIF(p_fk_srv_cube_tsg_db_scr,'');
		p_option_name := NULLIF(p_option_name,'');
		p_xf_atb_typ_name := NULLIF(p_xf_atb_typ_name,'');
		p_xk_atb_name := NULLIF(p_xk_atb_name,'');
		OPEN l_cube_cursor FOR
			SELECT
...
CREATE PROCEDURE bot.determine_position_sva (
			p_cube_sequence INOUT NUMERIC,
			p_cube_pos_action IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
		...
		LOOP
			...
			ELSE
				...
				FOR r_sva IN 
					...
					ORDER BY cube_sequence
				LOOP
					UPDATE bot.v_service_argument SET
						cube_sequence = l_cube_count
...
AS $BODY$
	...
	BEGIN
		...
		x_fk_srv_name := NULLIF(x_fk_srv_name,'');
		x_fk_srv_cube_tsg_db_scr := NULLIF(x_fk_srv_cube_tsg_db_scr,'');
		x_option_name := NULLIF(x_option_name,'');
		...
		IF COALESCE(p_cube_pos_action, ' ') NOT IN  ('A', 'B', 'F', 'L') THEN
			RAISE EXCEPTION 'Invalid position action: %', p_cube_pos_action;
		END IF;
		CALL bot.determine_position_sva  (l_cube_sequence, p_cube_pos_action, x_fk_typ_name, x_fk_srv_name, x_fk_srv_cube_tsg_db_scr, x_option_name, x_xf_atb_typ_name, x_xk_atb_name);
		UPDATE bot.v_service_argument SET
			cube_sequence = l_cube_sequence
		WHERE fk_typ_name = p_fk_typ_name
		  AND fk_srv_name = p_fk_srv_name
		  ...
		  AND xk_atb_name = p_xk_atb_name;
		IF NOT FOUND THEN
			RAISE EXCEPTION 'Type service_argument not found';
		END IF;
	END;
$BODY$;

CREATE PROCEDURE bot.insert_sva (
			p_cube_pos_action IN VARCHAR,
			p_fk_bot_name IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
			p_cube_tsg_sva_type IN VARCHAR,
			p_option_name IN VARCHAR,
			p_input_or_output IN VARCHAR,
			p_xk_itp_name IN VARCHAR,
			p_xf_atb_typ_name IN VARCHAR,
			p_xk_atb_name IN VARCHAR,
			p_xk_ref_bot_name IN VARCHAR,
			p_xk_ref_typ_name IN VARCHAR,
			p_xf_ref_typ_name IN VARCHAR,
			p_xk_ref_sequence IN NUMERIC,
			x_fk_typ_name IN VARCHAR,
			x_fk_srv_name IN VARCHAR,
			x_fk_srv_cube_tsg_db_scr IN VARCHAR,
			x_option_name IN VARCHAR,
...
AS $BODY$
	...
	BEGIN
		...
		p_fk_srv_name := NULLIF(p_fk_srv_name,'');
		p_fk_srv_cube_tsg_db_scr := NULLIF(p_fk_srv_cube_tsg_db_scr,'');
		p_cube_tsg_sva_type := NULLIF(p_cube_tsg_sva_type,'');
		p_option_name := NULLIF(p_option_name,'');
		p_input_or_output := NULLIF(p_input_or_output,'');
		p_xk_itp_name := NULLIF(p_xk_itp_name,'');
		p_xf_atb_typ_name := NULLIF(p_xf_atb_typ_name,'');
		p_xk_atb_name := NULLIF(p_xk_atb_name,'');
		p_xk_ref_bot_name := NULLIF(p_xk_ref_bot_name,'');
		p_xk_ref_typ_name := NULLIF(p_xk_ref_typ_name,'');
		p_xf_ref_typ_name := NULLIF(p_xf_ref_typ_name,'');
		x_fk_typ_name := NULLIF(x_fk_typ_name,'');
		x_fk_srv_name := NULLIF(x_fk_srv_name,'');
		x_fk_srv_cube_tsg_db_scr := NULLIF(x_fk_srv_cube_tsg_db_scr,'');
		x_option_name := NULLIF(x_option_name,'');
		...
		END IF;
		CALL bot.determine_position_sva  (l_cube_sequence, p_cube_pos_action, x_fk_typ_name, x_fk_srv_name, x_fk_srv_cube_tsg_db_scr, x_option_name, x_xf_atb_typ_name, x_xk_atb_name);
		INSERT INTO bot.v_service_argument (
			cube_id,
			cube_sequence,
			fk_bot_name,
			fk_typ_name,
	...
	EXCEPTION
	WHEN unique_violation THEN
			RAISE EXCEPTION 'Type service_argument already exists';
	END;
$BODY$;

CREATE PROCEDURE bot.update_sva (
			p_fk_bot_name IN VARCHAR,
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
...
CREATE PROCEDURE bot.delete_sva (
			p_fk_typ_name IN VARCHAR,
			p_fk_srv_name IN VARCHAR,
			p_fk_srv_cube_tsg_db_scr IN VARCHAR,
			p_option_name IN VARCHAR,
